...
class Activation extends CI_Controller
{
    ...
    public function index(){
        ...
        //list the users
        $data['users'] = $this->Users_model->get_member();
        ...
        $data['title'] = 'Activation Users';
        $data['subtitle'] = '';
        $data['crumb'] = [
            'Activation' => '',
        ];
        
        $data['code_js'] = 'users/activation_codejs';
        $data['pengaturan'] = $this->Pengaturan_model->get_by_id_1();          
        $data['page'] = 'users/activation';
        $this->load->view('template/backend', $data);
    }

    public function json() {
        header('Content-Type: application/json');
        echo $this->Users_model->activation_json();
    }

    public function aktivasi($id){
		$hasil=200;
		$this->Users_model->get_aktivasi($id,$hasil);
		helper_log("edit", "Data sudah diperbaharui"); 
	  redirect('activation');
  }
    ...
    public function request() {
        ...
        // $data = array(
    	// 	'first_name' => $this->input->post('nama_aktivasi',TRUE),
    	// 	// 'activation_code' => $this->input->post('activation_code',TRUE),
    	// 	// 'tujuan' => $this->input->post('tujuan_daftar',TRUE),
	    // );
        //     $this->Users_model->update($this->input->post('id_request', TRUE), $data);
        //     $this->session->set_flashdata('message', 'Data Sedang diajukan');
        //     helper_log("edit", "Aktivasi ".$data['first_name']."sedang diajukan");             
            // redirect(site_url('dashboard/'.$this->input->post('id_request', TRUE)));
            $this->load->view('template/backend', 'dashboard/activation');
    }
    ...
    public function aktifbulk(){
        $data = $_POST['msg_'];
        $dataid = explode(',', $data);
        foreach ($dataid as $key => $value) {
            $this->Users_model->get_aktivasi($value);
            $this->session->set_flashdata('message', 'Data Berhasil diaktivasi');             
        }
        echo true;
    }    
    public function deaktvasibulk(){
        $data = $_POST['msg_'];
        $dataid = explode(',', $data);
        foreach ($dataid as $key => $value) {
            $this->Users_model->get_deaktivasi($value);
            $this->session->set_flashdata('message', 'Data Berhasil dideaktivasi');             
        }
        echo true;
    }    
